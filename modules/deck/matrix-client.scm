...
(define-method (matrix-client-main-loop (matrix-client <matrix-client>)
  ...
  (let* ((session (matrix-client-session matrix-client))
    ...
    (let* ((presence (state-presense state))
           (events   (assoc-ref presence "events")))
      ...
      (unless (zero? (vector-length events))
        (for-each (lambda (event)
                  ...
                  (vector->list events))))
    (let* ((rooms-state (state-rooms state))
           (invite      (assoc-ref rooms-state "invite"))
           (join        (assoc-ref rooms-state "join"))
           (leave       (assoc-ref rooms-state "leave")))
      ;; (format #t "rooms-state: ~a~%" rooms-state)
      (unless (and (null? invite)
                   (null? join)
                   (null? leave))
        (for-each (lambda (proc) (proc rooms-state))
